...
export default {
  ...
  created() {
    this.fetchData(), this.fetchmydata();
  },
  watch: {
    $route: "fetchData",
    $route: "fetchmyData",
  ...
  methods: {
    ...
    async fetchmydata() {
      ...
      this.datumreg = moment(parseInt(this.mydata[0].datumreg)).format(
        "DD.MM.YYYY"
      );
    },
    async updatedata() {
    ...
    show(data) {
      ...
      moment.locale("hr");
      let a = this.store.podaci.filter(
        (element) =>
          moment(element.datum_obavljanja_pocetak).format("MMMM YYYY") == data
      );
  ...
  computed: {
    month: function () {
      ...
      return [
        ...new Set(
          this.store.podaci.map((data) =>
            moment(data.datum_obavljanja_pocetak).format("MMMM YYYY")
          )
        ),
      ];
    },
